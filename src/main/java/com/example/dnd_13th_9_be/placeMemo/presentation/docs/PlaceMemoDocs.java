...
public interface PlaceMemoDocs {
  ...
  @Operation(summary = "장소 메모 단건 조회", description = "placeMemoId에 해당하는 장소 메모를 조회합니다.")
  ResponseEntity<ApiResponse<QueryPlaceMemoResponse>> findOne(
      @Parameter(hidden = true) @AuthenticationPrincipal UserPrincipalDto user,
      @Parameter(description = "장소 메모 ID", required = true, example = "10")
          @PathVariable("placeMemoId")
          Long placeMemoId);

  @Operation(summary = "장소 메모 삭제", description = "placeMemoId에 해당하는 장소 메모를 삭제합니다. (이미지 파일도 함께 정리)")
  ...
  ResponseEntity<ApiResponse<QueryPlaceMemoListResponse>> findAll(
      @Parameter(hidden = true) @AuthenticationPrincipal UserPrincipalDto user,
      @Parameter(description = "폴더 ID", required = true, example = "1") @PathVariable("folderId")
          Long folderId);
}
