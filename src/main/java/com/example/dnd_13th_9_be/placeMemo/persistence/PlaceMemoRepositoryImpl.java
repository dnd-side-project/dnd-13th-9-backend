...
public class PlaceMemoRepositoryImpl implements PlaceMemoRepository {
  ...
  public PlaceMemoModel save(Long userId, PlaceMemoModel placeMemoModel) {
    ...
    Folder folder =
        jpaFolderRepository
            .findById(placeMemoModel.getFolderId())
            .orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND_FOLDER));

    return placeMemoConverter.from(
        jpaPlaceMemoRepository.save(placeMemoConverter.toEntity(placeMemoModel, user, folder)));
  }

  @Override
  public Optional<PlaceMemoModel> findByIdAndUserId(Long placeMemoId, Long userId) {
    return jpaPlaceMemoRepository
  ...
  public List<PlaceMemoModel> findByFolderIdAndUserId(Long folderId, Long userId) {
    return jpaPlaceMemoRepository.findByFolderIdAndUserIdOrderByIdDesc(folderId, userId).stream()
        .map(placeMemoConverter::from)
        .toList();
  }
