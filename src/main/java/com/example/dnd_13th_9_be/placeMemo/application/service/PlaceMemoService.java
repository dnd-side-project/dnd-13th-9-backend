...
public class PlaceMemoService {
    ...
    public CreatePlaceMemoResponse create(final Long userId, final CreatePlaceMemoRequest request){
        ...
        List<String> urls = (request.images() == null ? List.<MultipartFile>of() : request.images())
                .stream()
                .filter(f -> !f.isEmpty())
    ...
    public QueryPlaceMemoResponse findOne(final Long placeMemoId, final Long userId){
        PlaceMemoModel memoModel = placeMemoRepository.findByIdAndUserId(placeMemoId, userId)
    ...
    @Transactional
    public void update(final Long placeMemoId, final Long userId, final UpdatePlaceMemoRequest request){
        List<String> uploadedUrls = request.newImages() == null
                ? List.of()
    ...
    public QueryPlaceMemoListResponse findAll(final Long folderId, final Long userId){
        List<QueryPlaceMemoResponse> items = placeMemoRepository.findByFolderIdAndUserId(folderId, userId)
                .stream()
