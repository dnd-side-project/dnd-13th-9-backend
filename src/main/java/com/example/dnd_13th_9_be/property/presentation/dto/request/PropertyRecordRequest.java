...
public record PropertyRecordRequest(
    @EnumValid(enumClass = FeelingType.class, message = "feeling 값은 GOOD, SOSO, BAD 중 하나여야 합니다")
    FeelingType feeling,

    @NotNull(message = "매물명은 필수입니다")
    @Length(min = 1, max = 10, message = "매물명은 1~10자 이하여야 합니다")
    String propertyName,

    @Length(min = 1, max = 80, message = "메모는 1~80자 이하여야 합니다")
    String memo,

    @Length(min = 1, max = 255, message = "참고 링크는 1~255자 이하여야 합니다")
    String referenceUrl,

    @NotNull(message = "지번 주소는 필수 입니다")
    @Length(min = 1, max = 50, message = "지번 주소는 1~50자 이하여야 합니다")
    String address,

    @Length(min = 1, max = 50, message = "상세 주소는 1~50자 이하여야 합니다")
    ...
    @NotNull(message = "계약 형태는 필수 입니다")
    @EnumValid(enumClass = ContractType.class, message = "contractType 값은 MONTHLY_RENT, JEONSE, PURCHASE 중 하나여야 합니다")
    ContractType contractType,

    @EnumValid(enumClass = HouseType.class, message = "houseType 값은 OFFICETEL, ONEROOM, APARTMENT, VILLA, COLIVING, GOSIWON, BOARDING, ETC 중 하나여야 합니다")
    HouseType houseType,

    @NotNull(message = "억 단위는 필수 값입니다")
    @Min(value = 0, message = "억 단위는 필수 값입니다")
    @Max(value = 1000, message = "억 단위는 1000억 이상이 될 수 없습니다")
    Integer depositBig,

    @NotNull(message = "만원 단위는 필수 값입니다")
    @Min(value = 0, message = "만원 단위는 필수 값입니다")
    ...
    @Length(min = 1, max = 50, message = "입주 가능 시기는 1~50자 이하여야 합니다")
    String moveInInfo,

    @Length(min = 1, max = 200, message = "필수 확인 메모는 1~200자 이하여야 합니다")
    String requiredCheckMemo,

    @Valid
    List<ChecklistMemoRequest> categoriesMemo,

    @NotNull(message = "계획 인덱스 값은 필수 입니다")
    @Min(value = 1, message = "유효한 계획 인덱스 값이 아닙니다")
    Long planId,

    @NotNull(message = "폴더 인덱스 값은 필수 입니다")
    @Min(value = 1, message = "유효한 폴더 인덱스 값이 아닙니다")
    Long folderId
